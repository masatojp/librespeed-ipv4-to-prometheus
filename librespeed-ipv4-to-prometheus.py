...
def data():
    ...
    while True:
        ...
        speedtest = subprocess.run("/usr/local/bin/librespeed_speedtest-cli/librespeed-cli --json --share --chunks 1000 --timeout 60 --secure --local-json /usr/local/bin//librespeed_speedtest-cli/custom-sv-ipv4.json --telemetry-json /usr/local/bin//librespeed_speedtest-cli/telemetry.json", shell=True, stdout=PIPE, stderr=PIPE, text=True)
...
ping_gauge = Gauge('my_home_internet_ping_inonius_ipv4', 'My Home Internet Ping')
download_gauge = Gauge('my_home_internet_download_inonius_ipv4', 'My Home Internet Download')
upload_gauge = Gauge('my_home_internet_upload_inonius_ipv4', 'My Home Internet Upload')
...
if __name__ == '__main__':
    ...
    with ThreadingHTTPServer(('0.0.0.0', 8081), MyHTTPRequestHandler) as server:
